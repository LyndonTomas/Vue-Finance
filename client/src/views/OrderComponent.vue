...
<!-- The FrontEnd -->
  <div id="Orders-Container" class="container">
        ...
        <img title="Order List Page" id="client_logo" src="../assets/client-logo.png">
    ...
    <div class="entries">
      <label for=""><h4><strong>Entries: </strong> {{orders.length}}</h4></label>
    </div>
    <!-- Date -->
    <span>
      <input id="dateInput" v-model="date" type="date">
      {{date}}
    </span>
    ...
    <table id="orderTable" class="table w-auto table-responsive-xl ">
      ...
      <tbody>
        ...
        >
          ...
          <td><strong>{{order.user.fullname.firstname}}&nbsp;{{order.user.fullname.lastname}}</strong></td>
          <td
          ><a :href="'https://override-syntax.herokuapp.com/item/' + order.item.id">{{order.item.name}}</a></td>
          ...
          <td>&#8369; &nbsp; <strong>{{(order.item.quantity*order.item.price) + order.fee}}</strong></td>
          <td class="options">
            <button class="btn btn-success" title="Generate Receipt" v-on:click="showReceipt(
              order._id,
              order.created_at,
            ...
            <button title="Delete Order" class="btn btn-danger" v-on:click="deletePostV2(order.is_deleted)">
...
export default {
  ...
  methods:{
    ...
    async deletePost(id){
      var choice = confirm("Are you sure you want to permanently delete this order?")
      if(choice==true){
